...
procedure( bvSim( @optional ( jsonConfig nil ) )
    let( ( configDir pyRunner pId resJsonPath resTable itemsTable item101 item102 status101 status102 overallStatus )
        ...
        if( isFile( pyRunner ) then
            ...
            ; Use ipcBeginProcess with absolute path and unbuffered python (-u) for real-time live streaming into CIW
            pId = ipcBeginProcess(
...
old_pattern = r'procedure\(\s*bvSimProcessData[\s\S]*?\)\s*;\s*end\s*procedure'
alt_pattern = r'procedure\(\s*bvSim\(\s*@optional\s*\(\s*jsonConfig\s*nil\s*\)\s*\)[\s\S]*?\)\s*;\s*end\s*procedure'
...
for p in target_files:
    if os.path.exists(p):
        ...
        if re.search(old_pattern, content):
            ...
            content = re.sub(alt_pattern, new_bvsim, content)
            open(p, "w").write(content)
            print(f"Replaced alt in {p}")
        else:
            open(p, "w").write(content + "\n\n" + new_bvsim)
            print(f"Appended in {p}")
