            ...
            if s_id == "power_up":
                ...
                if "I_ISAR" in actions:
                    ...
                    if "voltage_limiting" in actions["I_ISAR"]:
                        v_isar_limit = parse_unit_val(actions["I_ISAR"].get("voltage_limiting", 3.3))
                    elif "voltage" in actions["I_ISAR"]:
                        v_isar_limit = parse_unit_val(actions["I_ISAR"].get("voltage", 3.3))
...
analog begin
    ...
    // Dynamic Calibration Current with Compliance Voltage from cosim.oneTest.json ({i_isar_val*1e6:.2f} uA, Max {v_isar_limit:.2f} V)
    ...
    if ($abstime >= {t_trim_start:.6e} && $abstime < {t_measure_mode:.6e}) begin
        if (V(GPIO8, VSS_PCB) > {v_isar_limit:.4f})
...
analog begin
    ...
    if ($abstime >= {t_trim_start:.6e} && $abstime < {t_measure_mode:.6e}) begin
        ...
        if (V(GPIO8, aVss) > {v_isar_limit:.4f})
            I(GPIO8, aVdd) <+ (V(GPIO8, aVss) - {v_isar_limit:.4f}) / 1e-3;
    end else begin
        I(aVdd, GPIO8) <+ 0.0;
    end
...
"""
            # Search for py_tester.sv in various candidate directories
            candidate_sv = [
                os.path.abspath(os.path.join(netlist_dir, "../../..", "py_tester.sv")),
                os.path.abspath(os.path.join(netlist_dir, "..", "py_tester.sv")),
                os.path.abspath(os.path.join(netlist_dir, "py_tester.sv")),
                f"/home/lary/simulation/BVU025/BVU025A/ocean/BVU025_Lary/sim_TOP_cosim_python_A1_ocean/py_tester.sv",
                f"/home/lary/simulation/BVU025/BVU025A/ocean/BVU025_Lary/sim_TOP_cosim_python_A1/py_tester.sv",
                f"/home/lary/simulation/BVU025/BVU025A/{top_cell}/py_tester.sv"
            ]
            py_tester_sv_path = None
            for p in candidate_sv:
                if os.path.exists(p):
                    py_tester_sv_path = p
                    break
            if not py_tester_sv_path:
                py_tester_sv_path = candidate_sv[0]
...
"""
        ...
        candidate_c = [
            os.path.abspath(os.path.join(netlist_dir, "../../..", "py_bridge.c")),
            os.path.abspath(os.path.join(netlist_dir, "..", "py_bridge.c")),
            os.path.abspath(os.path.join(netlist_dir, "py_bridge.c")),
            f"/home/lary/simulation/BVU025/BVU025A/ocean/BVU025_Lary/sim_TOP_cosim_python_A1_ocean/py_bridge.c",
            f"/home/lary/simulation/BVU025/BVU025A/ocean/BVU025_Lary/sim_TOP_cosim_python_A1/py_bridge.c",
            f"/home/lary/simulation/BVU025/BVU025A/{top_cell}/py_bridge.c"
        ]
        py_bridge_path = None
        for p in candidate_c:
            if os.path.exists(p):
                py_bridge_path = p
                break
        if not py_bridge_path:
            py_bridge_path = candidate_c[0]

        # Clean any old/nonexistent py_bridge.c lines from xrunArgs
        clean_args_lines = []
        for l in args.splitlines():
            if "py_bridge.c" in l or "-I/usr/include/python" in l or "-lpython" in l or "-L/usr/lib" in l:
                continue
