# sim_TOP_cosim_python_TM14 Python OneTest 協同模擬說明

本目錄為 **情況 B：Python 作為 OneTest 測試執行器（Outer Orchestrator）** 之模擬環境。

所有產生的模擬腳本、波形資料庫（PSF）與測試量測報告均完全儲存於本資料夾（`sim_TOP_cosim_python_TM14/`）內。
...
```bash
cd ~/project/BVU025/python/sim_TOP_cosim_python_TM14
