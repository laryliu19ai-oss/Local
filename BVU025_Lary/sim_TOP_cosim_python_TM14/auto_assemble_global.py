...
def universal_assemble(netlist_dir, pattern_dir=None):
    ...
    try:
        ...
        top_cell = "sim_TOP_cosim_python_TM14"
        ...
        # Derive pattern candidate from cell name (e.g. sim_TOP_cosim_python_TM14 -> TM14)
        ...
        if "A1" in top_cell:
            alias_candidates.extend([
                ...
                "/home/lary/project/BVU025/SCH/cosim/pattern/TM14andTM15",
        ...
        candidates = [
            *alias_candidates,
            ...
            "c:/Antgravity/Local/BVU025_Lary/sim_TOP_cosim_python_TM14"
        ]
        ...
        print(f"  - Calibration Current: {i_isar_val*1e6:.2f} uA (Voltage Limit: {v_isar_limit:.2f} V, Trim Mode @ {t_trim_start*1e6:.1f} us ~ {t_measure_end*1e3:.2f} ms)")
...
analog begin
    ...
    if ($abstime >= {t_trim_start:.6e} && $abstime < {t_measure_end:.6e} && V(GPIO8, VSS_PCB) > {v_isar_limit:.4f}) begin
...
"""
        ...
        for l in args.splitlines():
            ...
            if s.startswith("-name "):
                clean_args_lines.append(f"-name {lib_name}.{top_cell}:config")
                continue
            if s.startswith("-top ") and not s.startswith("-top cds_globals"):
                clean_args_lines.append(f"-top {lib_name}.{top_cell}:schematic")
                continue
            if "-xmsimargs" in s:
                s = re.sub(r'-top\s+\S+:schematic', f'-top {lib_name}.{top_cell}:schematic', s)
                s = re.sub(r'-name\s+\S+:config', f'-name {lib_name}.{top_cell}:config', s)
