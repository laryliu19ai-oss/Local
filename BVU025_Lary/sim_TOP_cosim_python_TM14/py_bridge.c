...
void c_main_tester(void) {
    ...
    PyRun_SimpleString(
        ...
        "cur_dir = os.getcwd()\n"
        "search_paths = [cur_dir]\n"
        "pattern_tm14 = '/home/lary/project/BVU025/SCH/cosim/pattern/TM14'\n"
        "if os.path.exists(pattern_tm14): search_paths.append(pattern_tm14)\n"
        "for d in [os.path.join(cur_dir, p) for p in ['..', '../..', '../../..']]:\n"
        "    if os.path.exists(d): search_paths.append(os.path.abspath(d))\n"
        "for p in reversed(search_paths):\n"
        "    if os.path.exists(p) and p in sys.path: sys.path.remove(p)\n"
        "    if os.path.exists(p): sys.path.insert(0, p)\n"
        "print('[py_bridge.c] Python sys.path[0]:', sys.path[0])\n"
    );
