...
class OneTestRunner:
    ...
    def _resolve_netlist_dir(self) -> Optional[str]:
        ...
        # Fallback auto-provisioning from sibling design directories if available
        if self.sim_output_dir and os.path.exists(self.sim_output_dir):
            target_netlist = os.path.join(self.sim_output_dir, self.top_cell, "ams", "config", "netlist")
            for sibling in os.listdir(self.sim_output_dir):
                sib_netlist = os.path.join(self.sim_output_dir, sibling, "ams", "config", "netlist")
                if os.path.exists(sib_netlist) and sibling != self.top_cell:
                    print(f"[OneTest Sim] Auto-initializing netlist environment from {sibling}...")
                    try:
                        shutil.copytree(os.path.join(self.sim_output_dir, sibling), os.path.join(self.sim_output_dir, self.top_cell), dirs_exist_ok=True)
                        dinfo = os.path.join(target_netlist, "designInfo")
                        if os.path.exists(dinfo):
                            content = open(dinfo).read().replace(sibling, self.top_cell)
                            open(dinfo, "w").write(content)
                        return os.path.abspath(target_netlist)
                    except Exception as e:
                        print(f"[OneTest Sim] Warning during auto-provisioning: {e}")
        return None

    def _resolve_psf_dir(self) -> str:
        """Locate active PSF results directory dynamically."""
        netlist_dir = self._resolve_netlist_dir()
        candidates = [
            os.path.join(netlist_dir, "..", "psf") if netlist_dir else "",
            os.path.join(self.sim_output_dir, self.top_cell, "ams", "config", "psf") if self.sim_output_dir else "",
    ...
    def run_simulation(self) -> bool:
        ...
        clean_targets = [
            ...
            os.path.join(self.work_dir, "images", "cosim_waveform.png"),
        ...
        # 3. Execute xrun
        cmd = "xrun -f xrunArgs"
        print(f"[OneTest Sim] Executing simulation in: {netlist_dir}", flush=True)
        
        proc = subprocess.Popen(cmd, cwd=netlist_dir, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, bufsize=1)
        sim_output_lines = []
        if proc.stdout:
            for line in proc.stdout:
                line_str = line.rstrip()
                sim_output_lines.append(line_str)
                if any(k in line_str for k in ["Executing SAR", "Bit[", "Converged", "Transient Analysis", "Step '", "Simulation complete", "time ="]):
                    print("  " + line_str, flush=True)
        proc.wait()

        if proc.returncode != 0:
            print(f"\n[OneTest Sim] *** ERROR: Simulation exited with code {proc.returncode} ***\n", flush=True)
            for line in sim_output_lines[-20:]:
                print("  " + line, flush=True)
            self.generate_fail_report(f"Simulation execution failed with code {proc.returncode}")
            return False

        # 4. Sync PSF database to work_dir/psf
        tm14_psf = os.path.join(self.work_dir, "psf")
        active_psf = self._resolve_psf_dir()
        if active_psf and os.path.exists(active_psf) and active_psf != os.path.abspath(tm14_psf):
        ...
        # 5. Always capture waveform image if cosim.waveform.json exists
        ...
        if os.path.exists(wave_json_path):
            self.capture_waveform_from_json()
        ...
        for p in plot_list:
            for res_item in p.get("results", []):
                for sig_info in res_item.get("signals", []):
                    ...
                    sig_type = sig_info.get("type", "voltage").lower()
                    display_lbl = sig_info.get("display", sig_name)
                    
                    var_name = f"sig_{sig_idx}"
                    sig_idx += 1
                    clean_name = sig_name.lstrip("/")

                    if sig_type == "digital" or "[" in sig_name or "<" in sig_name:
                        sig_declarations.append(f"""
{var_name} = getData("{tc}.{sig_name}")
when(!{var_name} {var_name} = getData("{sig_name}"))
when(!{var_name} {var_name} = getData("/{sig_name}"))
when(!{var_name} {var_name} = getData("{tc}.Board.{sig_name}"))
""")
                    elif sig_type == "current" or "i(" in sig_name.lower():
                        sig_declarations.append(f"""
{var_name} = i("{sig_name}")
when(!{var_name} {var_name} = i("/{clean_name}"))
when(!{var_name} {var_name} = getData("{tc}.{clean_name}_$flow"))
when(!{var_name} {var_name} = getData("{tc}.Board.{clean_name}_$flow"))
when(!{var_name} {var_name} = getData("{tc}.Board.TOP.{clean_name}_$flow"))
when(!{var_name} {var_name} = IT("/{clean_name}"))
""")
                    else:
                        sig_declarations.append(f"""
{var_name} = v("{sig_name}")
when(!{var_name} {var_name} = v("/{clean_name}"))
when(!{var_name} {var_name} = getData("{tc}.{clean_name}"))
when(!{var_name} {var_name} = getData("{clean_name}"))
when(!{var_name} {var_name} = getData("{tc}.Board.{clean_name}"))
when(!{var_name} {var_name} = getData("{tc}.Board.TOP.{clean_name}"))
""")

                    plot_commands.append(f"""
when({var_name} plot({var_name} ?expr "{display_lbl}" ?strip {strip_num}))
""")
...
"""
        ...
        try:
            res = subprocess.run(
                ["xvfb-run", "-a", "bash", "-c", "source ~/.bashrc && ocean -replay /tmp/auto_wave_gen.ocn"],
                cwd="/tmp",
            ...
            if os.path.exists(img_file):
                ...
                print(f"[OneTest Sim] Waveform capture failed. Ocean exit code: {res.returncode}")
                print(f"[OneTest Sim] xvfb-run stdout:\n{res.stdout}")
                print(f"[OneTest Sim] xvfb-run stderr:\n{res.stderr}")
        except subprocess.TimeoutExpired as e:
            print(f"[OneTest Sim] Waveform capture timed out after {e.timeout}s.")
            print(f"[OneTest Sim] stdout:\n{e.stdout}")
            print(f"[OneTest Sim] stderr:\n{e.stderr}")
        except Exception as e:
