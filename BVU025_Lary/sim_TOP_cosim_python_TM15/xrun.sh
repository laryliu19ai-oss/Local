...
if [[ "$*" == *"xrunArgs"* ]]; then
    if [ -d "$CUR_DIR/digital/ihnl" ]; then
        python3 /home/lary/project/BVU025/SCH/cosim/pattern/TM15/auto_assemble_global.py "$CUR_DIR" "/home/lary/project/BVU025/SCH/cosim/pattern/TM15" >> /tmp/xrun_wrapper.log 2>&1
    elif [ -d "$CUR_DIR/ams/config/netlist/digital/ihnl" ]; then
        python3 /home/lary/project/BVU025/SCH/cosim/pattern/TM15/auto_assemble_global.py "$CUR_DIR/ams/config/netlist" "/home/lary/project/BVU025/SCH/cosim/pattern/TM15" >> /tmp/xrun_wrapper.log 2>&1
...
if [ $XRUN_EXIT -eq 0 ]; then
    # Sync PSF files to TM15 pattern psf directory if running from simulation directory
    if [ -d "$CUR_DIR/../psf" ]; then
        mkdir -p /home/lary/project/BVU025/SCH/cosim/pattern/TM15/psf
        cp -u "$CUR_DIR/../psf"/* /home/lary/project/BVU025/SCH/cosim/pattern/TM15/psf/ 2>/dev/null || true
